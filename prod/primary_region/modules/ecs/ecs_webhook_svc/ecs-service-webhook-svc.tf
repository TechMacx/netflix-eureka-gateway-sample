...
resource "aws_security_group" "webhook" {
  ...
  ingress {
    ...
    cidr_blocks = ["44.231.31.224/32", "44.235.206.45/32", "52.13.208.146/32"]
    description = "SOLID WhiteList IPs to Farget-Conteaners"
  }
...
resource "aws_ecs_service" "webhook" {
  ...
  load_balancer {
    ...
    container_name   = "${var.infra_env}-${var.proj_name}-Webhook" //container_name should be same as name of template file's "name" tag
    container_port   = var.webhook_app_port
  }
  depends_on = [aws_iam_role_policy_attachment.webhook_ecs_task_execution_role, aws_iam_role_policy_attachment.webhook_ecs_task_role] // aws_lb_listener.webhook_backend_tcp
}
...
resource "aws_iam_role" "webhook_ecs_task_role" {
  name               = "${var.infra_env}-${var.proj_name}-webhook-task-role-${cluster_name}" //TaskRole
...
resource "aws_iam_role" "webhook_ecs_task_execution_role" {
  name               = "${var.infra_env}-${var.proj_name}-webhook-task-execution-role-${cluster_name}" //TaskExecutionRole
